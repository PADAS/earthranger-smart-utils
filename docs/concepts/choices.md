...
sync**. For every choice-bearing SMART attribute, we derive a single stable
`field` name:
...
`signo_de_caza` sees the same option list — and in ER, every event type
built from that datamodel references the same Choice records.

The scope hash keeps sharing bounded:

- **Not across CAs.** Two conservation areas can define the same attribute
  key with different options; their field names differ by CA UUID.
- **Not across CMs.** A Configurable Model curates its own option subsets
  (filter, reorder, deactivate), so each CM's lists are scoped by CM UUID
  — including apart from the plain-DM lists of the same CA.

One exception stays per-event-type: the consolidate-mode **variant
discriminator** (`et{8-hex}_variant`) — its options *are* the variant
identities of one event type, so there is nothing to share.

!!! note "Legacy `et*_` fields"
    Before choice sharing, every event type got its own copy of each list,
    named `et{8-hex}_{attr_key}` with the hash over the event-type value.
    Re-syncing a site created under that scheme re-points the event-type
    schemas at the shared `dm*_` lists and leaves the old `et*_` lists in
    place (active but unreferenced). Historical events are unaffected —
    stored option **values** are identical under both schemes — and the
    orphaned lists can be cleaned up manually if desired.

!!! warning "Per-node CM curation is collapsed"
    SMART allows a CM to curate the *same* attribute differently per node
    (multiple `attributeConfig` entries for one `attributeKey`, referenced
    via each node's `configId`). The smartconnect parser currently flattens
...
"signo_de_caza": {
  ...
  "items": {
    "type": "string",
    "anyOf": [
      {"$ref": "/api/v2.0/schemas/choices.json?field=dm5e6b96f4_signo_de_caza"}
    ]
  }
}
```
